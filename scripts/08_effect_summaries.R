...
# # :: AIPTW -------------------------------------------------------------------
# set.seed(12345)
# source(here::here("scripts", "06_crossfit_AIPW.R"))
# 
# for (outcome in posttest_names) {
# 
# # Imputed outcome
#   tmp <- fit_AIPTW(outcome = outcome,
#                    data = dt_completecase)
#   
#   # Columns that are constant across the loop
#   sublist_tmp <- list( "Subset" = "Single Imputed",
#                        "Model" = "AIPTW"
#   )
#   
#   # Assign results to list
#   tmp_list <- append(list("Mean_diff" = tmp$estimates$RD[["Estimate"]],
#                           "Lower"     = tmp$estimates$RD[["95% LCL"]],
#                           "Upper"     = tmp$estimates$RD[["95% UCL"]],
#                           "Domain"    = identify_domain(outcome)),
#                      sublist_tmp)
...
# 
#   # Load utility functions for SPC estimation
# source(here::here("scripts", "SPC", "SPC_ft.R"))
# 
# set.seed(123)
# 
# # :::: Unadjusted -----------------------------------------------------------
# 
# 
# # Pre-allocate matrix for mean and SE estimates for three approaches (6 columns)
# # for two different number of bins
# RESULT <- matrix(0L, 2L, 6L)
# RESULT_list <- list("MA" = RESULT,
#                     "EN" = RESULT,
#                     "RE" = RESULT)
# 
# 
# for (domain in c("MA", "EN", "RE")) {
#   
#   posttest <- posttest_names[grep(domain, posttest_names)]
#   pretest  <- pretest_names[grep(domain, pretest_names)] 
#   
#   dt_completecase_Y <- dt_completecase[!is.na(get(posttest))]
#   
#   # Pretest
#   W.Origin <- dt_completecase_Y[[pretest]]
#   # Posttest
#   Y.Origin <- dt_completecase_Y[[posttest]]
#   
#   Trt <- dt_completecase_Y[["initiated_01"]]
#   
#   
#   
#   # Below is the proposed approach for flexible modeling
#   # where design matrices consisting of quantile memberships are used
#   
#   # For continuous covariates, need to discretize in bins. 
#   # This is just a loop to generate 5 and 10 bin analyses
#   for(ii in c(1,2)){
#     num.bin    <- as.numeric(ii==1)*5 + as.numeric(ii==2)*10 # Equal to num.bin <- ii*5. Unnecessary also could have used for(num.bin in c(5,10))
#     # This will generate (num.bin+1) quantiles of the outcome variable, and exclude the 0% and 100% quantiles.
#     # e.g. if num.bin = 3, this will generate 33.33 and 66.6% percentiles
#     # e.g. if num.bin = 5, this will generate the 20, 40, 60 and 80% percentiles
#     Bin0.Cut   <- quantile(W.Origin,seq(0,1,length=num.bin+1))[-c(1,1+num.bin)] # 0 means pretest
#     Bin1.Cut   <- quantile(Y.Origin,seq(0,1,length=num.bin+1))[-c(1,1+num.bin)] # 1 means posttest
#     # Take the vector and make it into a`n x 1` matrix
#     # Then, for each of the `n` rows, indicate which quantile are they above
#     # e.g. first value is 3.73, and 20% = -2.9; 40% = -0.9; 60% = 0.8; 80% = 2.7
#     # thus, the value has to be 4, as they are above the 4th cutoff, i.e. in the 5th bin
#     # You can also see it as a bin membership indicator but starting at 0.
...
#     
#     # Pre-allocate two `n x b` matrices, where `b` is number of bins
#     Ymat.D <- Wmat.D <- matrix(0,length(Disc.Y0),num.bin)
#     # Assign binary indicators of bin membership
#     # e.g. if Disc.Y0 == 4, assign 1 to the 5th column
#     for(jj in 1:num.bin){ 
#       Wmat.D[Disc.Y0==jj-1,jj] <- 1
#       Ymat.D[Disc.Y1==jj-1,jj] <- 1
#     }
#     # Is this needed? all.equal(cbind(Ymat.D), Ymat.D) is TRUE
...
#     # Now there's two 1's per row for all those that were not in the last bin
#     
#     # This is the factor by which we multiply the number of bins
#     FACTOR <- 2
#     # Generate quantiles as before, but now for the `FACTOR*num.bin`
#     # e.g. with FACTOR = 2 and num.bin = 5, we have the 10,20,30,40,50,60,70,80,90 percentiles (as we still remove 0 and 100)
...
#     # As above, generate numeric indicators for bin membership
...
#     # Pre-allocate as above,`n x b` matrix of zeroes. `b` is number of bins
...
#     }
#     message(all.equal(Ymat.2.D, cbind(Ymat.2.D))) # Check if this is actually needed
...
#                           Ymat=Ymat.D1, # The design matrix for the posttest
#                           Wmat=Wmat.2.D, # The design matrix for the pretest
#                           penalty=10^(-6))
#     # Outcome Bridge Regression
#     Result.OR.D <- SPC.OR(Y.Origin,Trt,
#                           Ymat=Ymat.2.D,
#                           Wmat=Wmat.D)
#     
#     # Doubly Robust
#     # Note that for some reason the factor multiplied bins is used in the
#     # postest for outcome bridge regression and in the pretest for the propensity score
...
#                           Wmat.OR=Wmat.D,
#                           penalty=10^(-6)) 
#     # Have to see in the definition of SPC.OR if it allows for
#     # covariate adjustment set definition as splines or so
#     
#     
#     # Allocate results into the pre-allocated matrix
#     # Each row is for number of bins tried (e.g. 5 and 10 in the original code)
#     # Each column is for pairs of Estimate and SE (three pairs: EPS, OR, DR)
#     RESULT_list[[domain]][ii,] <- c( Result.PS.D$Est,Result.PS.D$SE,
#                                 Result.OR.D$Est,Result.OR.D$SE,
#                                 Result.DR.D$Est,Result.DR.D$SE )
#     # This is the only place where ii is used not in reference to num.bin,
#     # but could still be done with `which()`
#     
#   }
#   
# }
# 
#   # Create list with raw SPC estimates for each approach for log purposes
# raw_RESULTS_list <- list("EPS" = Result.PS.D,
#                          "OR" = Result.OR.D,
#                          "DR" = Result.DR.D)
#   # Save output objects
# saveRDS(RESULT_list, file = here::here("output", "SPC_RESULT_list.RDS"))
# saveRDS(raw_RESULTS_list, file = here::here("output", "SPC_raw_RESULT_list.RDS"))
# 
#   # Add column names to results within each domain
# for (i in 1:length(RESULT_list)) {
#   colnames(RESULT_list[[i]]) <- c("EPS-Est", "EPS-SE",
#                                   "OR-Est", "OR-SE",
#                                   "DR-Est", "DR-SE")
#   
# }
# 
# 
#   # For all three approaches in SPC
# for (approach in c("EPS", "OR", "DR")) {
#   # All SPC analyses are in a complete case subset
#   sublist_tmp <- list( "Subset" = "Complete")
#   # Add the specific approach to the model
#   sublist_tmp <- append(list("Model" = paste0("Unadjusted SPC-", approach))
#                         , sublist_tmp)
#   
#   tmp_estimate <- grep(paste0(approach, "-Est"), colnames(RESULT_list[[domain]]))
#   tmp_SE <- grep(paste0(approach, "-SE"), colnames(RESULT_list[[domain]]))
#   row_bins <- 1L
#   
#   # For all domains
#   for (domain in c("EN", "RE", "MA")) {
#     tmp_list <- append(list(  "Mean_diff" = RESULT_list[[domain]][row_bins, ][[tmp_estimate]],
#                               "Lower" = RESULT_list[[domain]][row_bins, ][[tmp_estimate]] -
#                                 1.96*RESULT_list[[domain]][row_bins, ][[tmp_SE]],
#                               "Upper" = RESULT_list[[domain]][row_bins, ][[tmp_estimate]] +
#                                 1.96*RESULT_list[[domain]][row_bins, ][[tmp_SE]],
#                               "Domain" = domain),
#                        sublist_tmp)
#     
...
# 
# # :::: Adjusted -----------------------------------------------------------------
#   # We know from the BART fitting that interactions are inconsequential
#   # Thus we will omit them
# set.seed(12345)
# 
# # You need to get rid of 1 bin membership column AND the intercept
#   # Else you run into collinear systems of equations
#   # As the code in SPC_ft by default adds intercept in model specifications
#   # (have to check the above but makes sense as input are just vectors of variables)
# 
# formula_str <- NULL
# # Partition outcome_covariates
# 
...
# start_time <- Sys.time()
# for (domain in c("MA", "EN", "RE")) {
#   # Store names of corresponding pretest and posttest for current domain
#   posttest <- posttest_names[grep(domain, posttest_names)]
#   pretest  <- pretest_names[grep(domain, pretest_names)] 
#   
#   # Exclude missing posttest observations
...
#   
#   # Create the design matrix
#     # This has to be done inside the loop since
#     # each outcome will have different number of rows and
#     # the cbind has to match number of rows of matrices later
#   covariate_design_mat <- model.matrix(formula_obj, data = dt_completecase_Y)
#   # Exclude pretest in this loop from the covariate matrix
#   covariate_design_mat <- covariate_design_mat[,-which(colnames(covariate_design_mat) == pretest)]
#   # Exclude intercept as the code in the SPC functions will introduce it
#   covariate_design_mat <- covariate_design_mat[, !colnames(covariate_design_mat) %in% "(Intercept)"]
#   # Pretest
#   W.Origin <- dt_completecase_Y[[pretest]]
#   # Posttest
#   Y.Origin <- dt_completecase_Y[[posttest]]
...
#     }
#     # Is this needed? all.equal(cbind(Ymat.D), Ymat.D) is TRUE
#     Ymat.D <- cbind(Ymat.D,  covariate_design_mat)  # Discretized Y ; used in EPS estimation 
#     Wmat.D <- cbind(Wmat.D,  covariate_design_mat)  # Discretized W ; used in ORB estimation
#     Ymat.D1 <- Ymat.D        
#     Ymat.D1[,num.bin] <- 1   # make the last bin as the reference for EPS estimation
#     # Now there's two 1's per row for all those that were not in the last bin
#     
...
#     }
#     message(all.equal(Ymat.2.D, cbind(Ymat.2.D))) # Check if this is actually needed
#     Ymat.2.D <- cbind(Ymat.2.D[,-1], covariate_design_mat)  # Discretized W ; used in EPS estimation 
#     Wmat.2.D <- cbind(Wmat.2.D[,-1], covariate_design_mat)  # Discretized Y ; used in ORB estimation
#     
#     # Extended Propensity Score
#     Result.PS.D <- SPC.PS(Y.Origin,Trt,
#                           Ymat=Ymat.D1, # The design matrix for the posttest
...
# end_time <- Sys.time()
# # Create list with raw SPC estimates for each approach for log purposes
# raw_adj_RESULTS_list <- list("EPS" = Result.PS.D,
#                          "OR" = Result.OR.D,
#                          "DR" = Result.DR.D)
# # Save output objects
# saveRDS(RESULT_list, file = here::here("output", "SPC_adj_RESULT_list.RDS"))
# saveRDS(raw_adj_RESULTS_list, file = here::here("output", "SPC_raw_adj_RESULT_list.RDS"))
# 
# # Add column names to results within each domain
# for (i in 1:length(RESULT_list)) {
#   colnames(RESULT_list[[i]]) <- c("EPS-Est", "EPS-SE",
...
#   sublist_tmp <- list( "Subset" = "Complete")
#   # Add the specific approach to the model
#   sublist_tmp <- append(list("Model" = paste0("Adjusted SPC-", approach))
#                         , sublist_tmp)
#   
#   tmp_estimate <- grep(paste0(approach, "-Est"), colnames(RESULT_list[[domain]]))
#   tmp_SE <- grep(paste0(approach, "-SE"), colnames(RESULT_list[[domain]]))
...
# 
# 
# # Save effect_summaries so far as to not re-compute
#   saveRDS(effect_summaries, file = here::here("output", "figure3_effects_NewData.RDS"))
# 

# :: Others ---------------------------------------------------------------

# # Create estimand variable 
# effect_summaries$estimand <- "SATE"
# effect_summaries[grepl("SPC", subsetXmodel)]$estimand <- "SATT"

# # Old plot
# fig3 <- ggplot(data = effect_summaries[!(grepl("EPS", # Omit reporting for OR and EPS to not clutter the plot
#                                                subsetXmodel) |
#                                            grepl("OR",
#                                                  subsetXmodel)) ],
#        aes(x = Mean_diff,
...
#              color = "gray50",
#              size = linewidth_plot) +
#   geom_point(aes(shape = estimand),position = position_dodge(width = 0.4), size = 3) +
#   geom_linerange(position = position_dodge(width = 0.4), linewidth = linewidth_plot) +
#   coord_cartesian(xlim = c(-2L, 2L)) + 
#   labs(x = "Difference between posttest and pretest national test raw scores",
#        y = "Data subset and \nmodel specification") +
#   theme(axis.text.y = element_text(hjust = 0)) +
#   khroma::scale_color_highcontrast()
...
# Attempt 1
# Having different rows for mean, lower and upper, with parameter specifying it
# effect_summaries <- list("Value" = vector(mode = "numeric"),
#      "Domain" = factor(NA_character_,
#                        levels = c("MA", "RE", "EN")
#                        # labels = c("Numeracy",
#                        #            "Reading",
...
#      "Subset" = factor(NA_integer_,
#                        levels = c("Complete", "Imputed")),
#      "Model" = vector(mode = "character")) |> data.table::as.data.table()
# 
# 
# effect_summaries <- effect_summaries[-1] 
# 
...
# for (outcome in posttest_names) {
#   tmp <- crude_difference(posttest = outcome)
#     # Columns that are constant across the loop
#   sublist_tmp <- list( "Subset" = "Complete",
#                        "Model" = "Unadjusted"
#                      )
#   
#   dif <- append(list("Value" = tmp$observed_difference,
#                      "Domain" = identify_domain(outcome),
#                      "Parameter" = 1L),
#                 sublist_tmp
#   )
#   
#   low <- append(list("Value" = tmp$lower,
#                      "Domain" = identify_domain(outcome),
#                      "Parameter" = 2L),
#                 sublist_tmp,
#   )
#   up  <- append(list("Value" = tmp$upper,
#                      "Domain" = identify_domain(outcome),
#                      "Parameter" = 3L),
#                 sublist_tmp,
#   )
#   
#   effect_summaries <- rbind.data.frame(effect_summaries, rbind.data.frame(dif, low, up)) |>  as.data.table()
# }
# 
#   # For some reason the typing is lost in the process
# effect_summaries$Parameter <- factor(effect_summaries$Parameter,
#                                      levels = c(1, 2, 3),
#                                      labels = c("Mean difference",
#                                                 "Lower",
#                                                 "Upper"))
